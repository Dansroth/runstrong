...
const INSIGHTS = {
  ...
  dragcurl:    { why: 'Elbows travel backward instead of staying pinned, which biases the long head of the biceps — the part that makes the peak.', deep: 'Dragging the bar up the torso puts the shoulder into slight extension, and the biceps long head crosses the shoulder, so it is the head that lengthens and works hardest there. It is a strict lift by construction: there is nowhere for a hip swing to go. Arm size, not running.' },
  concurl:     { why: 'The strictest curl there is — one arm, elbow braced, nothing to cheat with.', deep: 'Bracing the elbow on the inner thigh removes every source of momentum, so the biceps gets exactly the reps you paid for and the weaker arm cannot hide behind the stronger one. Useful precisely when fatigue makes a standing curl start to wobble.' },
  ...
  diamondpu:   { why: 'Triceps work that needs no equipment at all — the fallback when the gym is shut or you are away.', deep: 'Hands together under the sternum turns a push-up into a triceps press. It will not load like a close-grip bench, but it keeps the pattern alive on a week where the alternative is skipping it, and reps close to failure grow muscle at light loads too [H3].' },
  ...
  walkinglunge:{ why: 'Single-leg work that travels — every step is a whole stride cycle under load.', deep: 'Stepping through rather than returning to the start means the front leg has to absorb the landing and drive the body forward, which is the mid-stance and push-off of running with a dumbbell added. Hard on balance and on the glutes of the standing leg, and a genuinely different demand from a split squat held in place.' },
  slpress:     { why: 'Heavy single-leg loading with the balance taken out — one leg, full effort, no wobble.', deep: 'The machine removes every stability limiter, so the working leg can be loaded far heavier than any free-weight single-leg lift allows. That makes it the honest test of a side-to-side strength difference, and the tool for closing one.' },
  donkeycalf:  { why: 'Hips hinged forward puts the calf into a deeper stretch than a standing raise reaches.', deep: 'Bending at the hips lengthens the gastrocnemius across the knee and the ankle at once, so the bottom of each rep is a longer stretch than an upright raise gives. Straight-knee work, so it belongs with the standing raises rather than the seated ones.' },
  smithcalf:   { why: 'Standing calf work with the balance removed, so the load can climb.', deep: 'The fixed bar path means the only limit is the calves, not the wobble — which matters on a muscle that responds to load and takes a lot of it. Straight knee keeps it on the gastrocnemius.' },
